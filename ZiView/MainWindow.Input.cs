...
{
    /// <summary>
    /// MainWindow partial（コア）: マウスドラッグ・ズーム/パン、キーボードショートカットの一括ディスパッチ、
    /// 各種チェックボックス/スライダーの設定変更ハンドラ、サイドバー/ボトムバーのアニメーション、
    /// 右クリック/ハンバーガーメニュー、設定ウィンドウ起動、通知バッジを担当する。
    /// 回転・トーンカーブ・ルーペ・ページ位置微調整・GPU監視はそれぞれ専用ファイル
    /// （MainWindow.Rotation.cs / MainWindow.ToneCurve.cs / MainWindow.Loupe.cs /
    /// MainWindow.PageOffset.cs / MainWindow.GpuMonitor.cs）を参照。
    /// </summary>
